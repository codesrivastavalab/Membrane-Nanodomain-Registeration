...
// System Variables
  ...
  int population[2] = {5000,5000};                                              // number of lipids of each species in order : DPPC, D23
  double salpha[2] = {0.3074681, 0.1692048};                                    // average site variable of species in same order
  //double length[2] = {16, 22};                                                  // length of tail in the same order (currently unused)
  double unsat[2] = {0.0, 0.9545454};                                           // sum(pos)/numlength for each species in the same order

  //Monte Carlo simulation parameters
  int steps = 10000000;                                                         // steps per set of variables

  //Output parameters
  int interval = 5000;                                                          // print the system configuration every 'interval' steps
  ...
  for(tem = 0; tem<1; tem++){                                                   // increments through the entries in the temperature array
    for(Spl = 0; Spl<1; Spl++){                                                 // increments through the entries in the VSp array
      for(Sin = 0; Sin<4; Sin++){                                               // increments through the entries in the VSi array
        for(Vi00 = 0; Vi00<3; Vi00++){                                          // increments through the entries in the Viaa array
          for(Vi11 = 0; Vi11<3; Vi11++){                                        // increments through the entries in the Vibb array
              ...
              for(Vp00 = 0; Vp00<5; Vp00++){                                    // increments through the entries in the Vpaa array
                for(Vp11 = 0; Vp11<=Vp00; Vp11++){                              // increments through the entries in the Vpbb array such that Vpbb <= Vpaa
                  for(Vp10 = 0; Vp10<=Vp11; Vp10++){                            // increments through the entries in the Vpab array such that Vpab <= Vpbb
                    ...
                    // Setting parameters for current point in parameter space
                    ...
                    FILE *fp;
                    fp = fopen(buffer, "w+");
                    fprintf(fp,"Parameters in this run : Temp = %lf, VSp[0] = %lf, VSi[0] = %lf, Vinter[0][0] = %lf, Vinter[1][1] = %lf, Vinter[0][1] = %lf, Vplane[0][0] = %lf, Vplane[1][1] = %lf, Vplane[0][1] = %lf\n",kT,VSplane,VSinter,Vinter[0][0],Vinter[1][1],Vinter[0][1],Vplane[0][0],Vplane[1][1],Vplane[0][1]);
                    ...
                    for(i=0;i<m;i++){            // with a pop-cap check so that the filled lattice has the required populations of each species
                      for(j=0;j<n;j++){
                    ...
                    // Initial calculation of total system energy
                    ...
                    while(t<steps){
                      ...
                      if(t%5000==0){                                                    // Correcting drift due to numerical errors (if any)
                        energy = energy_calc(upper,lower);
                      }

                      if(t%interval==0){                                                // prints system config and energy every 'interval' steps
                        fprintf(fp,"move = %d, energy = %lf\n",t,energy);
                        print_config(upper, lower, fp);
                      }
                      t++;
                    }

                    // Wrap Up of run for current point in parameter space
...
double energy_calc(int** upper,int** lower)
{
  ...
  for(i=0;i<m;i++){
    for(j=0;j<n;j++){
      ...
      // In-plane part of the Hamiltonian
      for(k=0;k<4;k++){
        x = in[k]; y = jn[k];
        t = lower[x][y];
      ...
      // Interleaflet part of the Hamiltonian
  ...
  // Rescaling energy to reduced units
  H = H/epsilon;
  return H;
}
...
double mc_move(int** upper, int** lower, int x1, int y1, int x2, int y2, int flag)
{
  ...
  else{ // switch back the interchanged sites if move isn't accepted
    if(flag==0){
      lower[x2][y2] = tl1; lower[x1][y1] = tl2;
    }
    if(flag==1){
...
double local_energy(int** upper, int** lower, int x1, int y1, int* x1n, int* y1n, int flag){
  ...
  for(i=0;i<9;i++){
    ...
    // interleaflet entropic interaction of (x,y) and orthogonal neighbours in the opposite leaflet
    if(i<=3 || i==8){
      count[0] = 0.0; count[1] = 0.0; frac[0] = 0.0; frac[1] = 0.0;
      int f = 1 - flag;
